...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.18.json', '候选产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.18', '显示名正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 492, '四百九十二事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E492', '事件锚点范围为E01-E492');
...
ok(contract.required.terminal_hook_event === 'E492' || contract.required.terminal_hook_event?.id === 'E492', 'E492为本版开放终点');
...
ok(conceptRouterContent.includes('E492'), '路由事件序列含E492');
...
for (const [index, eventId] of EVENT_IDS.entries()) {
  ...
  if (eventId === 'E492') ok(!content.includes('## 下一事件引入'), 'E492冻结终点不设下一事件引入');
...
for (const [name, npc] of optionalCharacters) {
  ...
  const compact = ['智脑', '视界之主', '老人', '夏娃', '康斯坦丁', '第一使徒', '孽主', '弑利亚', '大黑彘', '血娃娃与母亲', '未来画家', '艾泽法拉', '雷蒙·冬语', '以太', '光宴'].includes(name);
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.18')), '酒馆助手脚本命名含v0.18');
...
ok(!statusUiText.includes("from: 'E492'"), 'E492无推进按钮');
...
ok(bridgePairs.length === 491, '全卡桥共491对');
ok(statusUiText.includes("from: 'E491', to: 'E492'"), '状态栏桥对覆盖E491→E492');
...
ok(statusBridgeCount === 491, `状态栏桥对共491对（实际${statusBridgeCount}）`);
...
// ===== 开放终态收束 =====
ok(EVENT_IDS.includes('E492'), '开放终态事件E492已纳入事件序列');
