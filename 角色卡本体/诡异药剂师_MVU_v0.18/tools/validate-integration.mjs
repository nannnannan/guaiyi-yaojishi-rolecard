...
for (const file of ['src/scripts/mvu_loader.js', 'src/regex_scripts.json']) {
  ok(read(file) === fs.readFileSync(path.join(originalRoot, file), 'utf8').replaceAll('0.15.0', '0.18.0').replaceAll('v0.15', 'v0.18'), `未授权的状态或加载器变更：${file}`);
}
...
ok(Object.keys(contract.required.optional_characters).length === 18, '应登记18名可选NPC');
...
for (let current = 318; current <= 492; current++) {
  const calls = [];
...
for (const entry of book.entries.filter(entry => entry.extensions?.tavernweave?.route_kind === 'optional_npc' || entry.id === 306)) {
  ...
  for (const state of ['未触发', '预兆', '活跃', '完成', '变形', '取消']) ok(typeof ejs.render(content, localsFor(492, state)) === 'string', `${entry.comment}状态渲染失败`);
}
