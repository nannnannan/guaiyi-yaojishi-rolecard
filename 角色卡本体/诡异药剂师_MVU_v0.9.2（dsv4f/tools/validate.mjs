...
ok(manifest.packed_json === `dist/${DISPLAY_NAME.replace(/《|》/g, '')}.json` || manifest.packed_json === 'dist/诡异药剂师_v0.9.2.json', '产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.9.2', '显示名正确');
ok(profile.ui_variant === 'death_realm_four_fold', '四折叠死界UI配置正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 126, '一百二十六事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E126', '事件锚点范围为E01-E126');
...
ok(contract.required.terminal_hook_event === 'E126' || contract.required.terminal_hook_event?.id === 'E126', 'E126为阶段五收束终点');
...
ok(conceptRouterContent.includes('E126'), '路由事件序列含E126');

// ===== 事件素材 =====
const eventMaterialEntries = sourceBook.entries.filter(entry => entry.id >= MATERIAL_START && entry.id <= MATERIAL_END);
ok(eventMaterialEntries.length === 126 && eventMaterialEntries.every(entry => entry.enabled === false), '126条独立事件素材全部禁用');
...
ok(stage4ConceptEntries.length >= 20, `阶段四及五概念UID551起共${stage4ConceptEntries.length}条`);
...
for (const conceptEntry of allConceptEntries) {
  ...
  if (conceptTitle === '术语速查（E36-E64）') {
    ...
    ok(Array.isArray(eventIds) && eventIds.length > 0 && eventIds.every(id => EVENT_IDS.includes(id)), `${conceptEntry.comment}事件关联非空且只使用E01-E126`);
...
ok(Object.values(characterEventIds).every(eventIds => Array.isArray(eventIds)
  && eventIds.length > 0
  && eventIds.every(eventId => EVENT_IDS.includes(eventId))), '人物事件关联仅使用E01-E126且均非空');
...
for (const character of CHARACTERS) {
  ...
  ok(eventIds.every(eventId => EVENT_IDS.includes(eventId)), `${character}事件元数据只使用E01-E126`);
...
ok(initial.元数据.卡名 === DISPLAY_NAME && initial.元数据.版本 === VERSION, '初始变量元数据为v0.9.2');
ok(Object.keys(initial.事件.锚点状态).length === 126, '初始锚点126个');
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.9.2')), '酒馆助手脚本命名含v0.9.2');
...
// 状态栏桥对（取消态不自动推进、E126无推进按钮、E64→E65前件挂E63）
...
ok(!statusUiText.includes("from: 'E126'"), 'E126无推进按钮');
...
ok(bridgePairs.length === 125, '全卡桥共125对');
ok(statusUiText.includes("from: 'E125', to: 'E126'"), '状态栏桥对覆盖E125→E126');
const statusBridgeCount = [...statusUiText.matchAll(/from: '(E\d{2,3})', to: '(E\d{2,3})'/g)].length;
ok(statusBridgeCount === 125, `状态栏桥对共125对（实际${statusBridgeCount}）`);
...
// ===== 阶段六待续收束 =====
ok(EVENT_IDS.includes('E126'), '阶段五收束终态事件E126已纳入事件序列');
const e126Content = await readText('src/events/E126_银发女王萝莉诞生与六根源对峙.md');
ok(e126Content.includes('六根源对峙') && e126Content.includes('双生子'), 'E126包含阶段五终极大对峙与双生子重塑');
