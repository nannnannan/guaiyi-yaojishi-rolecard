...
ok(manifest.version === '0.6.0', 'manifest版本为0.6.0');
...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.6.json', '产物文件名正确');
ok(cardSource.name === '《诡异药剂师》v0.6', '显示名正确');
...
ok(contract.required.stage_scope.includes('第1至276章') && contract.required.stage_scope.includes('第150至276章'), '阶段范围已声明');
ok(contract.required.core_character_count === 17, '十七名核心人物');
ok(contract.required.event_ids.length === 35, '三十五事件锚点');
ok(contract.required.time_progression.includes('纯事件驱动'), '纯事件驱动已声明');
ok(contract.required.evil_value_scope.includes('小小') && contract.required.evil_value_scope.includes('爱丽丝') && contract.required.evil_value_scope.includes('恒为0'), '幼态恶堕锁零边界已声明');
...
ok(contract.required.concept_activation?.concept_count === 93, '概念双路激活契约覆盖93条');
...
ok(contract.required.concept_activation?.stage1_enriched_concept_start === 400
  && contract.required.concept_activation?.stage1_enriched_concept_end === 451
  && contract.required.concept_activation?.stage1_enriched_concept_count === 52, '阶段一详写契约覆盖UID400-451共52条');
...
ok(contract.required.event_context_windows?.material_entry_start === 300
  && contract.required.event_context_windows?.material_entry_end === 334
  && contract.required.event_context_windows?.material_entries_enabled === false, 'UID300-334仅作禁用构建素材');
ok(contract.required.character_activation?.router_entry_id === 399, '人物与概念复用预处理路由ID 399');
ok(contract.required.character_activation?.character_count === 17, '人物事件路由覆盖17名核心人物');
...
ok(packed.data?.character_version === '0.6.0', 'data版本正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.id === manifest.worldbook.stable_id, '打包世界书稳定ID正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.version === '0.6.0', '打包世界书版本正确');
...
ok(packedBook.entries.length === contract.required.worldbook_entry_count, '世界书包含167条维护条目');
...
ok(conceptEntries.length === 93, '事件概念ID 400-492共93条');
...
for (const conceptEntry of conceptEntries) {
  ...
  ok(eventIds.every(eventId => contract.required.event_ids.includes(eventId)), `${conceptEntry.comment}事件关联只使用E01-E35`);
...
for (const { entry: conceptEntry, content: conceptContent } of variantConceptEntries) {
  ...
  ok(conceptContent.length >= 900 && conceptContent.length <= 2600, `${conceptEntry.comment}变体形态正文900-2600字符（当前${conceptContent.length}）`);
  ...
  blocks.forEach((block, index) => {
    ...
    if (!gate.baseline) {
      ...
      ok(contract.required.event_ids.includes(gate.events[0]), `${conceptEntry.comment}变体块${index + 1}门控事件只使用E01-E35`);
...
const farEventState = Object.fromEntries(contract.required.event_ids.map(id => [id, { 状态: id === 'E35' ? '活跃' : '未触发' }]));
...
ok(characters.length === 17, '十七名核心人物');
ok(components.length === 6, '每名角色六个组件');
ok(characters.includes('爱丽丝') && !characters.includes('黑弦月'), '爱丽丝进入角色阵容且黑弦月仍是物品');

const characterIds = contract.required.character_entry_ids;
const characterEventIds = contract.required.character_event_ids;
exactKeys(characterEventIds, characters, '十七人事件关联契约');
ok(Object.values(characterEventIds).every(eventIds => Array.isArray(eventIds)
  && eventIds.length > 0
  && eventIds.every(eventId => contract.required.event_ids.includes(eventId))), '人物事件关联仅使用E01-E35且均非空');
...
for (const character of characters) {
  ...
  ok(eventIds.every(eventId => contract.required.event_ids.includes(eventId)), `${character}事件元数据只使用E01-E35`);
...
ok(initial.关系.爱丽丝.恶堕 === 0, '爱丽丝幼态外观阶段恶堕恒为0');
ok(initial.关系.爱丽丝.边界.includes('非性') && initial.关系.爱丽丝.边界.includes('幼态外观期间恶堕恒为0'), '爱丽丝幼态阶段只开放非性关系方向');
...
for (const [index, eventId] of contract.required.event_ids.entries()) {
  ...
  if (index >= 20) {
    ...
    ok(deformationLine.includes('变形收束') && !deformationLine.includes('预兆') && !deformationLine.includes('活跃'), `${eventId}把变形限定为收束态`);
  }
  ...
  if (index >= 20) ok(content.length >= 1150 && content.length <= 1800, `${eventId}完整事件源码保持v0.5同量级（当前${content.length}）`);
  for (const marker of eventFieldMarkers) ok(content.includes(marker), `${eventId}包含${marker}`);
}

const eventMaterialEntries = sourceBook.entries.filter(entry => entry.id >= 300 && entry.id <= 334);
ok(eventMaterialEntries.length === 35 && eventMaterialEntries.every(entry => entry.enabled === false), '35条独立事件素材全部禁用');
...
exactKeys(dependencyTable, contract.required.event_ids.slice(20), 'E21-E35局部依赖表');
...
exactKeys(initial.关系, characters, '十七人关系');
...
for (const token of ['《诡异药剂师》v0.6', '0.6.0', '收尾', '恶堕', ...characters]) {
  ok(schemaText.includes(token), `schema包含：${token}`);
}
ok(schemaText.includes('爱丽丝') && !schemaText.includes('黑弦月'), 'schema含爱丽丝且不把黑弦月建为关系角色');
...
ok(helperNames.includes('v0.6'), '酒馆助手脚本命名含v0.6');
ok(helperSource.length === 2, '内嵌酒馆助手脚本2个');
ok(regexSource.length === 5, '正则脚本5个（状态栏、变量块、思考块的提示词与显示隔离）');
const hideStatusRegex = regexSource.find(script => script.id === 'tavernweave-hide-status-prompt-v0.6');
...
const hideThinkPrompt = regexSource.find(script => script.id === 'tavernweave-hide-think-prompt-v0.6');
const hideThinkDisplay = regexSource.find(script => script.id === 'tavernweave-hide-think-display-v0.6');
...
const blackMoonConcept = await readText('src/concepts/物品/黑弦月.md');
ok(blackMoonCount > 0 && blackMoonConcept.includes('实体已经到达药剂店') && blackMoonConcept.includes('保持封装、尚未正式启用') && blackMoonConcept.includes('阶段二没有开箱、启动、出战'), '黑弦月保持到店封装且阶段二不启用');
ok(sourceBook.entries.some(entry => entry.comment === '[角色]爱丽丝') && !sourceBook.entries.some(entry => entry.comment === '[角色]黑弦月'), '世界书注册爱丽丝且不把黑弦月建为角色');
ok(Object.prototype.hasOwnProperty.call(initial.关系, '爱丽丝') && !Object.prototype.hasOwnProperty.call(initial.关系, '黑弦月'), '关系变量含爱丽丝且无黑弦月');
...
ok(updateRulesText.includes('三十五个锚点状态'), '更新规则锚点计数为三十五个');
...
ok(!e35Content.includes('## 下一事件引入') && e35Content.includes('## 阶段3待续钩子') && !e35Content.includes('E36'), 'E35为阶段二终点且不创建E36');
...
ok(mainlineText.includes('E35') && mainlineText.includes('阶段二收束'), '上下文窗口明确E35阶段二边界');
...
ok(mainlineText.includes('### E20→E21') && !mainlineText.includes('### E35→'), '衔接段新增E20→E21且E35无后续');
