...
function runCoreContract(card, contract, manifest, { record = true } = {}) {
  ...
  const characterEntries = entries.filter(entry => entry.comment.startsWith('[角色]'));
  const characterNames = characterEntries.map(entry => entry.comment.slice(4));

  check(card.spec === 'chara_card_v3' && card.spec_version === '3.0', '角色卡符合 chara_card_v3 / 3.0');
  check(characterEntries.length === contract.required.core_character_count, '恰好打包七名核心角色');
  ...
  check(scripts.some(script => script.id === 'tavernweave-mvu-loader-v0.1' && script.enabled), 'MVU 固定版本加载器已嵌入并启用');
  check(scripts.some(script => script.id === 'tavernweave-mvu-schema-v0.1' && script.enabled), 'MVU Zod Schema 已嵌入并启用');
...
assert(manifest.deliverables.length === 1 && manifest.deliverables[0] === manifest.packed_json, 'v0.1 只声明 JSON 交付物');
...
const loader = scripts.find(script => script.id === 'tavernweave-mvu-loader-v0.1');
const schema = scripts.find(script => script.id === 'tavernweave-mvu-schema-v0.1');
...
const statusRegex = regexScripts.find(script => script.id === 'tavernweave-status-ui-v0.1');
const hideRegex = regexScripts.find(script => script.id === 'tavernweave-hide-mvu-v0.1');
...
const characterFiles = contract.required.core_characters.map(name => `src/characters/${name}.md`);
for (const [index, characterFile] of characterFiles.entries()) {
  const expected = (await text(characterFile)).trim();
  const packed = entries.find(entry => entry.comment === `[角色]${contract.required.core_characters[index]}`).content;
  assert(expected === packed, `${contract.required.core_characters[index]}人物设定与源码一致`);
}
...
missingDependency.data.extensions.tavern_helper.scripts = missingDependency.data.extensions.tavern_helper.scripts.filter(script => script.id !== 'tavernweave-mvu-loader-v0.1');
