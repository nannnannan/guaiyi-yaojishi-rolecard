...
ok(manifest.packed_json === `dist/${DISPLAY_NAME.replace(/《|》/g, '')}.json` || manifest.packed_json === 'dist/诡异药剂师_v0.9.3.json', '产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.9.3', '显示名正确');
...
ok(initial.元数据.卡名 === DISPLAY_NAME && initial.元数据.版本 === VERSION, '初始变量元数据为v0.9.3');
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.9')), '酒馆助手脚本命名含v0.9');
