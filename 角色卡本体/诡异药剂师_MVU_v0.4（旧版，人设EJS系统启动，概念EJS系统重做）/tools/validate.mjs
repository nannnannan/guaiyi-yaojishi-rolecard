...
ok(manifest.version === '0.4.0', 'manifest版本为0.4.0');
...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.4.json', '产物文件名正确');
ok(cardSource.name === '《诡异药剂师》v0.4', '显示名正确');
...
ok(packed.data?.character_version === '0.4.0', 'data版本正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.id === manifest.worldbook.stable_id, '打包世界书稳定ID正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.version === '0.4.0', '打包世界书版本正确');
...
ok(packedBook.entries.length === 49, '世界书包含49条维护条目');
...
for (const [index, eventId] of contract.required.event_ids.entries()) {
  ...
  ok(defaultLen >= 200 && defaultLen <= 400, `${eventId}默认走向200-400字（当前${defaultLen}）`);
...
for (const token of ['《诡异药剂师》v0.4', '0.4.0', '收尾', '恶堕', '左左', '血锯', '血衣女士', '小小', '人偶夫人', '泰坦头颅', '巫神头颅', '小宝贝']) {
...
ok(helperNames.includes('v0.4'), '酒馆助手脚本命名含v0.4');
...
ok(blackMoonCount > 0 && blackMoonCount <= 20, '黑弦月仅以“未登场/租借在途”语境保留');
...
ok(updateRulesText.includes('`/事件/锚点状态/{事件ID}/状态`置为`预兆`'), '更新规则要求预兆同步锚点状态');
