...
const BIG_ANCHORS = new Set(['E47', 'E60', 'E61', 'E63', 'E77', 'E80', 'E94']);
const HOOK_EVENTS = new Set(['E64', 'E96']);
...
ok(manifest.packed_json === `dist/${DISPLAY_NAME.replace(/《|》/g, '').replace(/v0\.8/, 'v0.8')}.json` || manifest.packed_json === 'dist/诡异药剂师_v0.8.json', '产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.8', '显示名正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 96, '九十六事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E96', '事件锚点范围为E01-E96');
ok(CHARACTERS.length === 27, '二十七名核心人物');
...
ok(contract.required.terminal_hook_event?.id === 'E96' && contract.required.terminal_hook_event?.six_state === false && contract.required.terminal_hook_event?.advance_button === false, 'E96为无六态无推进的待续钩子');
ok(contract.required.concept_activation?.stage4_note?.includes('删除字数上限'), '阶段四概念删除字数上限已声明');
...
ok(conceptRouterContent.includes('routeWorlds.has(String(entry?.world ?? ""))'), '路由只处理承载UID399的本卡世界书');
ok(conceptRouterContent.includes('E96'), '路由事件序列含E96');

// ===== 事件素材 =====
const eventMaterialEntries = sourceBook.entries.filter(entry => entry.id >= MATERIAL_START && entry.id <= MATERIAL_END);
ok(eventMaterialEntries.length === 96 && eventMaterialEntries.every(entry => entry.enabled === false), '96条独立事件素材全部禁用');
ok(eventMaterialEntries.every(entry => entry.id === MATERIAL_START + EVENT_IDS.indexOf(entry.comment.match(/E\d{2}/)[0])), '事件素材ID与锚点索引一一对应');
...
for (const [index, eventId] of EVENT_IDS.entries()) {
  ...
  if (HOOK_EVENTS.has(eventId)) {
    if (eventId === 'E64') {
      ...
      ok(content.includes('待续钩子') && content.includes('不创建E65'), 'E64开放钩子不创建E65');
    } else {
      ok(!content.includes('## 下一事件引入') && !content.includes('- 默认走向：') && !content.includes('- 完成条件：'), 'E96不设六态、引入与默认走向');
      ok(content.includes('阶段待续钩子') && content.includes('不创建E97') && !content.includes('<%'), 'E96只列阶段待续钩子且无EJS');
    }
    continue;
  }

  ok(content.includes('<%_') && content.includes('getvar("stat_data.事件.锚点状态.'), `${eventId}蓝灯含EJS状态门槛（六态）`);
  ok(content.includes('## 下一事件引入'), `${eventId}含下一事件引入`);
  ...
  const upper = BIG_ANCHORS.has(eventId) ? 800 : 500;
  ok(defaultLen >= 200 && defaultLen <= upper, `${eventId}默认走向200-${upper}字（当前${defaultLen}）`);
  ok(content.length >= 300 && content.length <= 5000, `${eventId}事件源码长度300-5000字符（当前${content.length}）`);
}
...
ok(stage4ConceptEntries.length === 20, `阶段四概念UID551起共20条（实际${stage4ConceptEntries.length}）`);
...
const allConceptEntries = [...stage1ConceptEntries, ...stage3ConceptEntries, ...stage4ConceptEntries];
for (const conceptEntry of allConceptEntries) {
  ...
  if (conceptTitle === '术语速查（E36-E64）') {
    ...
    ok(Array.isArray(eventIds) && eventIds.length > 0 && eventIds.every(id => EVENT_IDS.includes(id)), `${conceptEntry.comment}事件关联非空且只使用E01-E96`);
...
exactKeys(characterEventIds, CHARACTERS, '二十七人事件关联契约');
ok(Object.values(characterEventIds).every(eventIds => Array.isArray(eventIds)
  && eventIds.length > 0
  && eventIds.every(eventId => EVENT_IDS.includes(eventId))), '人物事件关联仅使用E01-E96且均非空');
ok(new Set(Object.values(CHARACTER_IDS)).size === CHARACTERS.length, '二十七名人物UID互不重复');
...
for (const character of CHARACTERS) {
  ...
  ok(eventIds.every(eventId => EVENT_IDS.includes(eventId)), `${character}事件元数据只使用E01-E96`);
...
for (const character of ['万机之神', '巨像之脑', '血肉支配者']) {
  const entry = sourceBook.entries.find(item => item.comment === `[角色]${character}`);
  const source = await expectedEntryContent(entry);
  ok(source.includes('未登场'), `${character}使用未登场锁`);
  ok(source.includes('不详'), `${character}未登场部分使用不详`);
}
...
for (const character of candidateNames) {
  ok(CHARACTERS.includes(character), `${character}候选对象属于二十七人`);
...
ok(initial.元数据.卡名 === DISPLAY_NAME && initial.元数据.版本 === VERSION, '初始变量元数据为v0.8');
ok(Object.keys(initial.事件.锚点状态).length === 96, '初始锚点96个');
exactKeys(initial.事件, ['锚点状态', '唯一活跃事件', '近期预兆', '最近结果', '阶段总结', '原创重大事件待定队列'], '事件');
exactKeys(initial.林恩, LIN_EN_FIELDS, '林恩');
exactKeys(initial.关系, CHARACTERS, '二十七人关系');
...
ok(initial.林恩.年龄 === 17 && initial.林恩.等级 === 2, '林恩年龄等级保持原文面板');
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.8')), '酒馆助手脚本命名含v0.8');
...
// 状态栏桥对（取消态不自动推进、E96无推进按钮、E64→E65前件挂E63）
...
ok(!statusUiText.includes("from: 'E96'"), 'E96无推进按钮');
...
ok(bridgePairs.length === 95, '全卡桥共95对');
ok(statusUiText.includes("from: 'E95', to: 'E96'"), '状态栏桥对覆盖E95→E96');
const statusBridgeCount = [...statusUiText.matchAll(/from: '(E\d{2})', to: '(E\d{2})'/g)].length;
ok(statusBridgeCount === 95, `状态栏桥对共95对（实际${statusBridgeCount}）`);
...
for (const file of modelVisibleFiles) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (const [pattern, name] of [[CHAPTER_PATTERN, '第X章'], [SUMMARY_PATTERN, '小总结/大总结'], [STAGE_NUMBER_PATTERN, '阶段几']]) {
      if (pattern.test(line)) forbiddenViolations.push(`${file}:${index + 1} [${name}] ${line.trim().slice(0, 60)}`);
    }
  }
}
for (const [pattern, name] of [[CHAPTER_PATTERN, '第X章'], [SUMMARY_PATTERN, '小总结/大总结'], [STAGE_NUMBER_PATTERN, '阶段几']]) {
  if (pattern.test(packedText)) {
    const m = packedText.match(new RegExp(`.{0,30}${pattern.source}.{0,30}`, 'g')) ?? [];
    for (const hit of m.slice(0, 5)) forbiddenViolations.push(`dist [${name}] ${hit.replace(/\n/g, ' ').slice(0, 80)}`);
  }
}
if (forbiddenViolations.length > 0) remainingFailures.push(...forbiddenViolations.map(v => `禁词负例 ${v}`));

// ===== 阶段五泄漏负例 =====
ok(!/E97/.test((await Promise.all(modelVisibleFiles.map(file => readText(file, false)))).join('\n').replace(/不创建E97/g, '')), '正文不含E97（除“不创建E97”否定式）');
ok(!/(?:第\s*560|[五六七八九十]百.*章)/.test(packedText) || !/第\s*(?:5[6-9][0-9]|[6-9][0-9]{2,})\s*章/.test(packedText), '正文不含第560章及之后内容');
const e96Content = await readText('src/events/E96_待续钩子.md');
ok(e96Content.includes('巨像之脑苏醒后的态度') && e96Content.includes('六诅咒残留') && e96Content.includes('咒瞳修复'), 'E96只列阶段五封存线程');
ok(!e96Content.includes('E97') || e96Content.includes('不创建E97'), 'E96不创建E97');
