...
const BIG_ANCHORS = new Set(['E358', 'E360', 'E366', 'E382', 'E385', 'E391', 'E47', 'E60', 'E61', 'E63', 'E77', 'E80', 'E94', 'E224', 'E226', 'E228', 'E236', 'E239', 'E242', 'E245', 'E253', 'E254', 'E257', 'E260', 'E261', 'E265', 'E268', 'E272', 'E274', 'E283', 'E287', 'E297', 'E307', 'E314', 'E315', 'E317']);
...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.17.json', '候选产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.17', '显示名正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 434, '四百三十四事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E434', '事件锚点范围为E01-E434');
...
ok(contract.required.terminal_hook_event === 'E434' || contract.required.terminal_hook_event?.id === 'E434', 'E434为本版开放终点');
...
ok(conceptRouterContent.includes('E434'), '路由事件序列含E434');
...
for (const [index, eventId] of EVENT_IDS.entries()) {
  ...
  if (eventId === 'E434') ok(!content.includes('## 下一事件引入'), 'E434冻结终点不设下一事件引入');
  ...
  ok(content.length >= 300 && content.length <= 10000, `${eventId}事件源码长度300-7000字符（当前${content.length}）`);
}
...
ok(stage10ConceptEntries.length === 25, `v0.15阶段十新增概念共25条（实际${stage10ConceptEntries.length}）`);
...
for (const conceptEntry of allConceptEntries) {
  ...
  if (conceptTitle === '术语速查（E36-E64）') {
    ...
    ok(Array.isArray(eventIds) && eventIds.length > 0 && eventIds.every(id => EVENT_IDS.includes(id)), `${conceptEntry.comment}事件关联非空且只使用E01-E434`);
...
ok(new Set(stage10ConceptKeys).size === stage10ConceptKeys.length, 'v0.15新增概念keys包内唯一');
ok(stage10ConceptKeys.every(key => !preV012ConceptKeys.has(key)), 'v0.15新增概念keys不与既有概念冲突');
...
ok(Object.values(characterEventIds).every(eventIds => Array.isArray(eventIds)
  && eventIds.length > 0
  && eventIds.every(eventId => EVENT_IDS.includes(eventId))), '人物事件关联仅使用E01-E434且均非空');
...
for (const character of CHARACTERS) {
  ...
  ok(eventIds.every(eventId => EVENT_IDS.includes(eventId)), `${character}事件元数据只使用E01-E434`);
...
// 可选NPC保留双路由；精简NPC用单文件，其余角色沿用六组件，不进入核心关系表。
const optionalCharacters = Object.entries(contract.required.optional_characters ?? {});
ok(optionalCharacters.length === contract.required.character_activation.optional_character_count, '可选NPC数量与契约一致');
ok(new Set(optionalCharacters.map(([, npc]) => npc.entry_id)).size === optionalCharacters.length, '可选NPC UID互不重复');
for (const [name, npc] of optionalCharacters) {
  const entry = sourceBook.entries.find(item => item.id === npc.entry_id);
  ok(Boolean(entry), `${name}可选NPC存在`);
  ok(entry.comment === `[角色]${name}` && entry.constant === false, `${name}以独立角色关键词条目注册`);
  ok(Array.isArray(entry.keys) && entry.keys.length > 0, `${name}保留原生关键词路径`);
  ok(entry.extensions?.tavernweave?.route_kind === 'optional_npc', `${name}路由类型正确`);
  ok(JSON.stringify(entry.extensions?.tavernweave?.event_ids) === JSON.stringify(npc.event_ids), `${name}事件路由与契约一致`);
  ok(npc.event_ids.length > 0 && npc.event_ids.every(id => EVENT_IDS.includes(id)), `${name}事件门槛在本版范围内`);
  ok(npc.relation_registered === false && !CHARACTERS.includes(name), `${name}未加入核心关系矩阵`);
  const compact = ['智脑', '视界之主', '老人', '夏娃', '康斯坦丁', '第一使徒', '孽主', '弑利亚', '大黑彘', '血娃娃与母亲', '未来画家', '艾泽法拉'].includes(name);
  ok((npc.profile_format === 'compact_npc') === compact, `${name}资料格式符合人物范围`);
  ok(entry.content_files?.length === (compact ? 1 : 6) && JSON.stringify(entry.content_files) === JSON.stringify(npc.content_files), `${name}资料路径与契约一致`);
  if (compact) {
    ok(entry.content_files[0] === `src/characters/${name}/NPC.md`, `${name}使用独立NPC文件`);
    ok(!/性格调色盘|三面性|二次解释|多阶段人设|角色速览|基础信息/u.test(await readText(entry.content_files[0])), `${name}不再套用六组件`);
  }
  ok(entry.extensions?.exclude_recursion === true && entry.extensions?.prevent_recursion === true, `${name}保留双递归保护`);
  for (const file of entry.content_files) ok((await readText(file)).length >= 80, `${name}组件非空：${file}`);
  const packedNpc = packedBook.entries.find(item => item.id === npc.entry_id);
  ok(packedNpc?.content === await expectedEntryContent(entry), `${name}打包资料与源码精确一致`);
}
// 拥星者保留既有UID和窗口，避免注册扩展意外改变老NPC身份。
...
if (optionalNpc && optionalNpcEntry) {
  ...
  ok(JSON.stringify(optionalNpcEntry.extensions?.tavernweave?.event_ids) === JSON.stringify(['E337', 'E338', 'E352']), '拥星者覆盖旧事件与E352撤退争议');
...
ok(initial.元数据.卡名 === DISPLAY_NAME && initial.元数据.版本 === VERSION, '初始变量元数据为v0.17');
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.17')), '酒馆助手脚本命名含v0.17');
...
// 状态栏桥对（取消态不自动推进、E434无推进按钮、E64→E65前件挂E63）
...
ok(!statusUiText.includes("from: 'E434'"), 'E434无推进按钮');
...
ok(bridgePairs.length === 433, '全卡桥共433对');
ok(statusUiText.includes("from: 'E433', to: 'E434'"), '状态栏桥对覆盖E433→E434');
ok(statusUiText.includes("from: 'E396', to: 'E397'"), '状态栏桥对覆盖E396→E397');
const statusBridgeCount = [...statusUiText.matchAll(/from: '(E\d{2,3})', to: '(E\d{2,3})'/g)].length;
ok(statusBridgeCount === 433, `状态栏桥对共433对（实际${statusBridgeCount}）`);
...
// ===== 开放终态收束 =====
ok(EVENT_IDS.includes('E434'), '开放终态事件E434已纳入事件序列');
const e434Content = await readText('src/events/E434_灭绝令与八旗牢笼.md');
ok(
  e434Content.includes('八旗')
    && e434Content.includes('灭绝')
    && e434Content.includes('镇旗'),
  'E434严格停在灭绝令与八旗牢笼',
);
ok(!e434Content.includes('## E435') && !e434Content.includes('E435·') && !e434Content.includes('下一事件引入'), 'E434未越界创建E435或后继引入');
const e396Content = await readText('src/events/E396_血肉承载与力量适应.md');
ok(e396Content.includes('## 下一事件引入（E397'), 'E396已引入E397');
