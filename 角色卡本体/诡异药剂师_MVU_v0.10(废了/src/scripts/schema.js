const BRIDGE_URLS = [
  'https://cdn.jsdelivr.net/gh/StageDog/tavern_resource@7f29257de3ffbd83d63bc37ca09f4d4ecad6ca0f/dist/util/mvu_zod.js',
  'https://testingcf.jsdelivr.net/gh/StageDog/tavern_resource@7f29257de3ffbd83d63bc37ca09f4d4ecad6ca0f/dist/util/mvu_zod.js',
];

async function loadSchemaBridge() {
  let lastError;
  for (const url of BRIDGE_URLS) {
    try {
      const module = await import(url);
      if (typeof module.registerMvuSchema === 'function') {
        return module.registerMvuSchema;
      }
    } catch (error) {
      lastError = error;
      console.warn(`[诡异药剂师v0.10] Zod 桥接加载失败，尝试备用源：${url}`, error);
    }
  }
  throw lastError ?? new Error('registerMvuSchema 不可用');
}

const score = z.coerce.number().transform(value => _.clamp(Math.round(value), 0, 100));
const shortText = z.string().max(320);
const mediumText = z.string().max(800);
const shortList = z.array(z.string().max(200)).max(16);
const sceneTime = shortText.refine(
  value => !/(?:星期[一二三四五六日天]|周[一二三四五六日天]|(?:第\s*)?(?:\d+|[一二三四五六七八九十百千万]+|好几|好多|很多|数|几|多)\s*天|昨天|昨日|昨夜|今天|今日|今早|今晨|明天|明日|次日|翌日|第二天|\d{4}\s*[-/.年]\s*\d{1,2}\s*[-/.月]\s*\d{1,2}\s*日?|\d{1,2}月\d{1,2}日)/.test(value),
  '场景时间只能使用事件标签与相对时段，不得写星期、精确日期、X天时长或昨天/昨日/昨夜/今天/今日/今早/今晨/明天/明日/次日/翌日/第二天',
);
const eventState = z.enum(['未触发', '预兆', '活跃', '变形', '完成', '取消']);
const survivalState = z.enum(['活动', '受伤', '失联', '受困', '休眠', '暂离', '敌对']);
const phaseNames = {
  S0: '药店接管期',
  S1: '怪物客户建立期',
  S2: '血衣与药剂风波期',
  S3: '魔人协会冲突期',
  S4: '猎头者家庭与师徒归来期',
  S5: '幸福之家与独立行医期',
  S6: '夜医启程与寂静镇灾变期',
  S7: '黑夜城授职与跨界伏笔期',
  S8: '蓝星召唤与寻亲期',
  S9: '黑夜城成长与人偶线期',
  S10: '宗教战争与疫医伪装期',
  S11: '疫医潜入与纳米风暴期',
  S12: '万机之神降临与根源混战期',
  S13: '战后修复与机械总攻期',
  S14: '利维坦潜入与疫医夺权期',
  S15: '第一腔室绝杀与主母双魂期',
  S16: '巨像意识空间与母树博弈期',
  S17: '巨像冲击地狱与双生子重塑期',
  S18: '六根源战场与机械狂潮期',
  S19: '战后撤离与巨像余波期',
  S20: '王后谈判与堕天使危机期',
  S21: '主母复苏与羽毛笔真身期',
  S22: '命运权柄与地狱通牒期',
  S23: '命运契约与蓝星召唤期',
  S24: '蓝星救援与双生子悬崖期',
};
const anchorTitles = {
  E01: '系统觉醒与开店',
  E02: '血娃娃上门与找妈妈任务',
  E03: '猪头魔与恶灵血婴',
  E04: '猎魔人治疗与传说左轮',
  E05: '不眠夜与小小牙科',
  E06: '诅咒清除能力与鬼婴塑形',
  E07: '血衣寻子与鬼婴萌化化解',
  E08: '暴击药剂与视界曝光',
  E09: '血衣鸿门宴与诅咒绑定',
  E10: '视界经营与玥玥治疗',
  E11: '血夜地狱行者与左左诞生',
  E12: '血肉灾变与左手共存',
  E13: '猎杀者夜袭与信使线',
  E14: '瘦长鬼影手术与024诊断',
  E15: '求援信与小小赴约',
  E16: '械魔大军与猎神者',
  E17: '小小介绍棒医生',
  E18: '倒吊塔决战与协会覆灭',
  E19: '猎头者家族绑定',
  E20: '师徒归来与阶段收束',
  E21: '魔鬼训练与首次独立出诊',
  E22: '幸福之家的血肉晚宴',
  E23: '异域同乡与爱丽丝初愈',
  E24: '诅咒复燃与古堡围猎',
  E25: '双面伯爵与心脑之汤',
  E26: '爱丽丝重生与母女终局',
  E27: '白逸遣返与九州异变',
  E28: '爱丽丝日常与十四病战蜥',
  E29: '传奇赠药与夜医启程',
  E30: '血月狼群与寂静镇试诊',
  E31: '哭泣小丑与血肉灾变',
  E32: '小丑余波与夜医接纳',
  E33: '夜医试炼与白夜旧梦',
  E34: '倒吊天使与夜医授职',
  E35: '左左拟化与蓝星鬼袭',
  E36: '左左醉酒与安全照护',
  E37: '白逸召唤降临与总局清剿',
  E38: '审问王蓓与根源关系网',
  E39: '暴食血祭与血娃娃回收',
  E40: '户籍锁定与血娃娃共情',
  E41: '白家休整与机械造物',
  E42: '跳车寻母与林樱初遇',
  E43: '回归汇报与被收教子',
  E44: '盗版史诗药剂与巨像脑情报',
  E45: '召唤四阶段与无瞳法阵',
  E46: '强制拟化与边界反击',
  E47: '群星圣堂实习与误麻院长',
  E48: '接诊喵喵与002盗书',
  E49: '犬皮改造真相与千人寻亲',
  E50: '猫娘药剂与喵喵破茧',
  E51: '黑弦月到货与保护契约',
  E52: '森一夜袭与四次重生',
  E53: '三重算计与平等契约',
  E54: '疫医暗面与最高级保护',
  E55: '喵喵收养与家人复仇',
  E56: '史莱姆整形与游荡者情报',
  E57: '整备出征与收编小宝贝',
  E58: '前线观战与银色幻想降临',
  E59: '斩臂反击与三根源驰援',
  E60: '圣子推定与圣徒叛变',
  E61: '突围反杀与夺萃之镰',
  E62: '折跃入城与疫医伪装',
  E63: '堡垒解围与欲望教派',
  E64: '诅咒之城余韵',
  E65: '疫医X博士潜入·指挥官拉拢示爱',
  E66: '电钻充能暴走·人偶情感暴涨',
  E67: '纳米风暴追杀·快感冲击蜂巢',
  E68: '血肉养蛊·病毒反噬纳米风暴',
  E69: '黑弦月签人偶之誓',
  E70: '撤离堡垒揭巨像之心',
  E71: '设伏反打银色幻想',
  E72: '核爆硬抗捕获机械姬',
  E73: '猎魔左轮处决与破防全部失败',
  E74: '快感冲击逼供缴双剑与龙狙',
  E75: '触发恶堕向机械姬调教任务',
  E76: '全城机械紊乱与死星结晶交易',
  E77: '万机之神投影降临贯穿取心',
  E78: '巨像网络苏醒与捣翻万机之神',
  E79: '黑弦月只身赴死与求婚被拒',
  E80: '直死书与六诅咒同归于尽',
  E81: '黑弦月肉身挡轰炸与解雇无效',
  E82: '白发异变与虚握差杀万机之神',
  E83: '艾雯爵士救场与八音盒扰泰坦',
  E84: '吞钱逼斗志与认义弟立执念',
  E85: '轮椅碰撞与漏电警告',
  E86: '工伤条款与同意治疗黑弦月',
  E87: '纽带比喻与无效解咒字据',
  E88: '恒星熔炉锻造人偶之臂',
  E89: '求左左协助解咒被拒',
  E90: '救回乱流深处的恶灵妹妹',
  E91: '妹妹误触诅咒释放与照护',
  E92: '强制调用左手解咒事故',
  E93: '入视界建群与喵喵参战',
  E94: '三蜂巢饱和轰炸与母树降临',
  E95: '银色幻想恨意暴增与总攻',
  E96: '血肉利维坦突现与战局逆转',
  E97: '三大根源混战与利维坦升空',
  E98: '赛博飞升升格与喵喵血仇',
  E99: '冤家组队与12小时禁杀契约',
  E100: '利维坦走廊潜伏与老六挂机',
  E101: '夺舍洛什与芬芳尖啸者嘴遁',
  E102: '机械姬恶堕大计与荣升执事',
  E103: '腐败执事生活与全厂物资收买',
  E104: '以M破防机械姬与苦肉计夺权',
  E105: '密室除患与全舰生化下毒',
  E106: '木偶咒印与人偶夫人真相',
  E107: '傀儡接管与双簧大戏诱敌',
  E108: '第一腔室掀牌与全舰异变灾变',
  E109: '利维坦鲸落与替身破死书',
  E110: '开颅刑讯与兄长残躯回溯',
  E111: '恶面堕天使降临与巨头噬神',
  E112: '以M亵渎神明与红茶包唤醒主母',
  E113: '巨像红芒爆发与深层空间降临',
  E114: '万机梦魇与解救巨像之舌',
  E115: '母树现身色诱与爆头自戕博弈',
  E116: '荒原漫步心智交锋与耍赖骑乘',
  E117: '往昔恐惧投影生命树与幼稚园',
  E118: '幻境以M快打与欲望母树交融',
  E119: '替死复活晋升与一丝神性觉醒',
  E120: '艾雯失忆死战与巨像之门开启',
  E121: '知识禁库渡鸦寓言与自我肢解',
  E122: '紫罗兰法阵决裂与剥离左左',
  E123: '巨像重组与破宇冲向地狱边界',
  E124: '直面狂乱色彩与极限断闸救赎',
  E125: '废墟渡鸦誓言与咒瞳绝唱挡刀',
  E126: '银发女王萝莉诞生与六根源对峙',
  E127: '六根源僵局与诅咒全面加深',
  E128: '支配者目的与母树第一绑定',
  E129: '泰坦暴怒降临与支配者追猎',
  E130: '巫神头颅复苏与家族重聚',
  E131: '母树临时同盟与未来筹码',
  E132: '机械狂潮解封与泰坦引流',
  E133: '血肉夺舰与集团撤离计划',
  E134: '全面狩猎与银色幻想终局变故',
  E135: '聚合体开颅与喵喵兄长残魂',
  E136: '最后一面与兄长永别',
  E137: '纳米血肉集群认亲与新族群',
  E138: '巨像罪疚与地狱追责誓言',
  E139: '王后真名揭晓与左左苏醒',
  E140: '王后归属谈判与巨像收容',
  E141: '巨像神殿掩埋与战舰返城',
  E142: '心灵深渊仪式与堕天使复苏',
  E143: '还书禁忌与羽毛笔失忆承诺',
  E144: '权柄异常与命运书写救城',
  E145: '堕天使决战与八音盒权柄造物',
  E146: '心灵深渊救援与主母归返',
  E147: '主母复苏与地狱反噬降临',
  E148: '神之方程式与凝视十七',
  E149: '重返002与认知空间驱逐',
  E150: '权柄病变与拒绝放弃',
  E151: '血肉真身接纳与平等信任',
  E152: '命运规则神与正式对话',
  E153: '古老狱规回应与生死反转',
  E154: '地狱惩罚预演与七日通牒',
  E155: '路西法前史与渡鸦指引',
  E156: '蓝星死界恶鬼与召唤预警',
  E157: '规避凝视的跨世界方案',
  E158: '百零七世界咒式与关系裂痕',
  E159: '羽毛笔重逢与生死命运契约',
  E160: '蓝星百鬼夜行与魔改义肢',
  E161: '尹依身世与死界恶鬼辨认',
  E162: '多次召唤失败与跨界失联',
  E163: '尹琴中弹与部分降临窗口',
  E164: '部分降临与恶灵聚合体镇压',
  E165: '尹依交易与尹琴急救',
  E166: '蓝星善后与夜医学徒去向',
  E167: '自毁争执与左左离家',
  E168: '双生关系危机与共同过夜',
  E169: '夜间越界与信任崩塌',
  E170: '清晨质问与餐刀悬崖',
};

const eventIds = Object.keys(anchorTitles);

function relationship(options = {}) {
  const attraction = options.attractionLocked
    ? z.literal(0)
    : score;
  const evil = options.evilLocked
    ? z.literal(0)
    : score;
  const lewdness = options.attractionLocked
    ? z.literal(0)
    : score;
  const emotional = options.emotionalOpen ? score : z.literal(0);
  const tame = options.a01Open ? score : z.literal(0);
  const collapse = options.a01Open ? score : z.literal(0);
  const hatred = options.a01Open ? score : z.literal(0);
  return z.object({
    解锁: z.boolean(),
    在场: z.boolean(),
    生存状态: survivalState,
    位置: shortText,
    处境: mediumText,
    关系类型: shortText,
    人物阶段: shortText,
    好感: score,
    信赖: score,
    戒备: score,
    吸引: attraction,
    关系创伤: score,
    恶堕: evil,
    涩涩度: lewdness,
    情感开发度: emotional,
    驯服度: tame,
    崩坏值: collapse,
    恨意值: hatred,
    可见迹象: shortList,
    边界: mediumText,
    关键记忆: shortList,
    最近互动: mediumText,
  });
}

const eventIdOrEmpty = z.enum([...eventIds, '']);

const anchor = z.object({
  标题: z.string().max(120),
  状态: eventState,
  收尾: z.boolean(),
});

const activeEvent = z.object({
  事件ID: eventIdOrEmpty,
  标题: z.string().max(140),
  地点: shortText,
  参与者: shortList,
  状态: z.enum(['活跃', '无']),
  紧迫度: z.enum(['无', '低', '中', '高', '极高']),
  模糊期限: shortText,
  进展: mediumText,
  幕后停止点: mediumText,
});

const omen = z.object({
  事件ID: eventIdOrEmpty,
  方向: mediumText,
  地点: shortText,
  参与者: shortList,
  紧迫度: z.enum(['无', '低', '中', '高', '极高']),
  模糊期限: shortText,
});

const result = z.object({
  事件ID: z.string().max(16),
  标题: z.string().max(140),
  结果: mediumText,
  世界影响: mediumText,
});

const pendingEvent = z.object({
  标题: z.string().max(140),
  长期影响: mediumText,
  前置条件: mediumText,
  紧迫度: z.enum(['低', '中', '高']),
});

const relationEdge = z.object({
  双方: z.tuple([z.string().max(40), z.string().max(40)]),
  关系类型: shortText,
  当前状态: mediumText,
  公开程度: z.enum(['林恩未知', '仅当事人知情', '局部公开', '公开']),
  最近变化: mediumText,
});

const Schema = z.object({
  元数据: z.object({
    卡名: z.literal('《诡异药剂师》v0.10'),
    版本: z.literal('0.10.0'),
  }),
  世界: z.object({
    阶段编号: z.enum(Object.keys(phaseNames)),
    阶段名称: z.enum(Object.values(phaseNames)),
    模糊阶段进度: z.enum(['初期', '推进中', '后期', '收束']),
    当前场景时间: sceneTime,
    地点: shortText,
    氛围: mediumText,
    大局线索: shortList,
  }),
  林恩: z.object({
    年龄: z.literal(18),
    身体状况: mediumText,
    当前身份: shortList,
    等级: z.number().int().min(1).max(999),
    技能: shortList,
    成就: shortList,
    图鉴已解锁: shortList,
    关键诅咒或契约: shortList,
    当前明确目标: mediumText,
    最近明确指令: mediumText,
    压力: score,
    道德: score,
    罪恶: score,
    复苏度: score,
    替死娃娃剩余: z.number().int().min(0).max(999),
    天使之吻状态: z.boolean(),
    根系网络权限: z.boolean(),
    巨像网络权限: z.boolean(),
    咒瞳状态: shortText,
    拟化技能: shortText,
  }),
  事件: z.object({
    锚点状态: z.object({
      E01: anchor,
      E02: anchor,
      E03: anchor,
      E04: anchor,
      E05: anchor,
      E06: anchor,
      E07: anchor,
      E08: anchor,
      E09: anchor,
      E10: anchor,
      E11: anchor,
      E12: anchor,
      E13: anchor,
      E14: anchor,
      E15: anchor,
      E16: anchor,
      E17: anchor,
      E18: anchor,
      E19: anchor,
      E20: anchor,
      E21: anchor,
      E22: anchor,
      E23: anchor,
      E24: anchor,
      E25: anchor,
      E26: anchor,
      E27: anchor,
      E28: anchor,
      E29: anchor,
      E30: anchor,
      E31: anchor,
      E32: anchor,
      E33: anchor,
      E34: anchor,
      E35: anchor,
      E36: anchor,
      E37: anchor,
      E38: anchor,
      E39: anchor,
      E40: anchor,
      E41: anchor,
      E42: anchor,
      E43: anchor,
      E44: anchor,
      E45: anchor,
      E46: anchor,
      E47: anchor,
      E48: anchor,
      E49: anchor,
      E50: anchor,
      E51: anchor,
      E52: anchor,
      E53: anchor,
      E54: anchor,
      E55: anchor,
      E56: anchor,
      E57: anchor,
      E58: anchor,
      E59: anchor,
      E60: anchor,
      E61: anchor,
      E62: anchor,
      E63: anchor,
      E64: anchor,
      E65: anchor,
      E66: anchor,
      E67: anchor,
      E68: anchor,
      E69: anchor,
      E70: anchor,
      E71: anchor,
      E72: anchor,
      E73: anchor,
      E74: anchor,
      E75: anchor,
      E76: anchor,
      E77: anchor,
      E78: anchor,
      E79: anchor,
      E80: anchor,
      E81: anchor,
      E82: anchor,
      E83: anchor,
      E84: anchor,
      E85: anchor,
      E86: anchor,
      E87: anchor,
      E88: anchor,
      E89: anchor,
      E90: anchor,
      E91: anchor,
      E92: anchor,
      E93: anchor,
      E94: anchor,
      E95: anchor,
      E96: anchor,
      E97: anchor,
      E98: anchor,
      E99: anchor,
      E100: anchor,
      E101: anchor,
      E102: anchor,
      E103: anchor,
      E104: anchor,
      E105: anchor,
      E106: anchor,
      E107: anchor,
      E108: anchor,
      E109: anchor,
      E110: anchor,
      E111: anchor,
      E112: anchor,
      E113: anchor,
      E114: anchor,
      E115: anchor,
      E116: anchor,
      E117: anchor,
      E118: anchor,
      E119: anchor,
      E120: anchor,
      E121: anchor,
      E122: anchor,
      E123: anchor,
      E124: anchor,
      E125: anchor,
      E126: anchor,
      E127: anchor,
      E128: anchor,
      E129: anchor,
      E130: anchor,
      E131: anchor,
      E132: anchor,
      E133: anchor,
      E134: anchor,
      E135: anchor,
      E136: anchor,
      E137: anchor,
      E138: anchor,
      E139: anchor,
      E140: anchor,
      E141: anchor,
      E142: anchor,
      E143: anchor,
      E144: anchor,
      E145: anchor,
      E146: anchor,
      E147: anchor,
      E148: anchor,
      E149: anchor,
      E150: anchor,
      E151: anchor,
      E152: anchor,
      E153: anchor,
      E154: anchor,
      E155: anchor,
      E156: anchor,
      E157: anchor,
      E158: anchor,
      E159: anchor,
      E160: anchor,
      E161: anchor,
      E162: anchor,
      E163: anchor,
      E164: anchor,
      E165: anchor,
      E166: anchor,
      E167: anchor,
      E168: anchor,
      E169: anchor,
      E170: anchor,
    }),
    唯一活跃事件: activeEvent,
    近期预兆: omen,
    最近结果: z.array(result).max(12),
    阶段总结: z.array(z.string().max(1000)).max(8),
    原创重大事件待定队列: z.array(pendingEvent).max(6),
  }),
  关系: z.object({
    左左: relationship(),
    血锯: relationship({ attractionLocked: true, evilLocked: true }),
    血衣女士: relationship(),
    小小: relationship({ attractionLocked: true, evilLocked: true }),
    人偶夫人: relationship(),
    爱丽丝: relationship({ attractionLocked: true, evilLocked: true }),
    白逸: relationship({ attractionLocked: true, evilLocked: true }),
    泰坦头颅: relationship({ attractionLocked: true, evilLocked: true }),
    巫神头颅: relationship({ attractionLocked: true }),
    小宝贝: relationship({ attractionLocked: true, evilLocked: true }),
    倒吊天使: relationship(),
    白夜: relationship({ attractionLocked: true, evilLocked: true }),
    渡鸦: relationship({ attractionLocked: true, evilLocked: true }),
    黑颅: relationship({ attractionLocked: true, evilLocked: true }),
    猪头屠夫: relationship({ attractionLocked: true, evilLocked: true }),
    哭泣小丑: relationship({ attractionLocked: true, evilLocked: true }),
    黑白小丑: relationship({ attractionLocked: true, evilLocked: true }),
    黑弦月: relationship({ attractionLocked: true, evilLocked: true, emotionalOpen: true }),
    喵喵: relationship({ attractionLocked: true, evilLocked: true }),
    林樱: relationship({ attractionLocked: true, evilLocked: true }),
    艾雯爵士: relationship({ attractionLocked: true, evilLocked: true }),
    羽毛笔: relationship(),
    a01银色幻想: relationship({ a01Open: true }),
    欲望母树: relationship(),
    万机之神: relationship({ attractionLocked: true, evilLocked: true }),
    巨像之脑: relationship({ attractionLocked: true, evilLocked: true }),
    血肉支配者: relationship({ attractionLocked: true, evilLocked: true }),
  }),
  角色关系: z.array(relationEdge),
  系统: z.object({
    当前任务: shortText,
    任务阶段: shortText,
    任务状态: z.enum(['进行中', '暂停', '完成', '无']),
    任务说明: mediumText,
    图鉴: shortList,
    成就: shortList,
    最近提示: mediumText,
    事件通知: mediumText,
    更新模式: z.literal('同轮更新；可选独立预设'),
  }),
}).superRefine((data, ctx) => {
  if (phaseNames[data.世界.阶段编号] !== data.世界.阶段名称) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['世界', '阶段名称'],
      message: '阶段编号与阶段名称不匹配',
    });
  }

  const anchorEntries = Object.entries(data.事件.锚点状态);
  const activeAnchors = anchorEntries.filter(([, value]) => value.状态 === '活跃');
  const activeEvent = data.事件.唯一活跃事件;
  const activeStateIsEmpty = activeEvent.状态 === '无';
  const activeEmptyIsCanonical = activeEvent.事件ID === ''
    && activeEvent.标题 === ''
    && activeEvent.地点 === ''
    && activeEvent.参与者.length === 0
    && activeEvent.紧迫度 === '无'
    && activeEvent.模糊期限 === ''
    && activeEvent.进展 === ''
    && activeEvent.幕后停止点 === '';
  const activeMatches = activeAnchors.length === 1
    && activeAnchors[0][0] === activeEvent.事件ID
    && activeEvent.状态 === '活跃';
  if ((activeStateIsEmpty && (activeAnchors.length !== 0 || !activeEmptyIsCanonical))
      || (!activeStateIsEmpty && !activeMatches)) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['事件', '唯一活跃事件'],
      message: '重大事件必须唯一活跃，并与锚点状态一致',
    });
  }

  for (const [eventId, title] of Object.entries(anchorTitles)) {
    const eventAnchor = data.事件.锚点状态[eventId];
    if (eventAnchor.标题 !== title) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['事件', '锚点状态', eventId, '标题'],
        message: `${eventId}标题必须保持稳定`,
      });
    }
    if ((eventAnchor.状态 === '未触发' || eventAnchor.状态 === '预兆') && eventAnchor.收尾) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['事件', '锚点状态', eventId, '收尾'],
        message: `${eventId}未触发或预兆时收尾必须为false`,
      });
    }
    if (['变形', '完成', '取消'].includes(eventAnchor.状态) && !eventAnchor.收尾) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['事件', '锚点状态', eventId, '收尾'],
        message: `${eventId}进入终态时收尾必须为true`,
      });
    }
  }

  if (activeEvent.事件ID && activeEvent.标题 !== anchorTitles[activeEvent.事件ID]) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['事件', '唯一活跃事件', '标题'],
      message: '活跃事件标题与事件ID不匹配',
    });
  }

  const omen = data.事件.近期预兆;
  const omenId = omen.事件ID;
  const omenAnchors = anchorEntries.filter(([, value]) => value.状态 === '预兆');
  const omenEmptyIsCanonical = omenId === ''
    && omen.方向 === ''
    && omen.地点 === ''
    && omen.参与者.length === 0
    && omen.紧迫度 === '无'
    && omen.模糊期限 === '';
  const omenMatches = omenAnchors.length === 1 && omenAnchors[0][0] === omenId;
  if ((!omenId && (omenAnchors.length !== 0 || !omenEmptyIsCanonical))
      || (omenId && !omenMatches)) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['事件', '近期预兆', '事件ID'],
      message: '近期预兆必须唯一对应预兆锚点；无预兆时所有详情字段必须清空',
    });
  }
});

const registerMvuSchema = await loadSchemaBridge();

$(() => {
  registerMvuSchema(Schema);
});
