...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.10.json', '产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.10', '显示名正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 170, '一百七十事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E170', '事件锚点范围为E01-E170');
...
ok(contract.required.terminal_hook_event === 'E170' || contract.required.terminal_hook_event?.id === 'E170', 'E170为本版开放终点');
...
ok(conceptRouterContent.includes('E170'), '路由事件序列含E170');
...
const allConceptEntries = [...stage1ConceptEntries, ...stage3ConceptEntries, ...stage4ConceptEntries, ...stage6ConceptEntries];
for (const conceptEntry of allConceptEntries) {
  ...
  if (conceptTitle === '术语速查（E36-E64）') {
    ...
    ok(Array.isArray(eventIds) && eventIds.length > 0 && eventIds.every(id => EVENT_IDS.includes(id)), `${conceptEntry.comment}事件关联非空且只使用E01-E170`);
...
ok(Object.values(characterEventIds).every(eventIds => Array.isArray(eventIds)
  && eventIds.length > 0
  && eventIds.every(eventId => EVENT_IDS.includes(eventId))), '人物事件关联仅使用E01-E170且均非空');
...
for (const character of CHARACTERS) {
  ...
  ok(eventIds.every(eventId => EVENT_IDS.includes(eventId)), `${character}事件元数据只使用E01-E170`);
...
ok(initial.元数据.卡名 === DISPLAY_NAME && initial.元数据.版本 === VERSION, '初始变量元数据为v0.10');
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.10')), '酒馆助手脚本命名含v0.10');
...
// 状态栏桥对（取消态不自动推进、E170无推进按钮、E64→E65前件挂E63）
...
ok(!statusUiText.includes("from: 'E170'"), 'E170无推进按钮');
...
ok(bridgePairs.length === 169, '全卡桥共169对');
ok(statusUiText.includes("from: 'E169', to: 'E170'"), '状态栏桥对覆盖E169→E170');
const statusBridgeCount = [...statusUiText.matchAll(/from: '(E\d{2,3})', to: '(E\d{2,3})'/g)].length;
ok(statusBridgeCount === 169, `状态栏桥对共169对（实际${statusBridgeCount}）`);
...
// ===== 开放终态收束 =====
ok(EVENT_IDS.includes('E170'), '开放终态事件E170已纳入事件序列');
const e170Content = await readText('src/events/E170_清晨质问与餐刀悬崖.md');
ok(
  e170Content.includes('持刀逼近')
    && e170Content.includes('刀锋尚未落下')
    && e170Content.includes('所有后果均保持未知'),
  'E170严格停在持刀逼近、刀锋尚未落下且所有后果未知',
);
ok(!e170Content.includes('E171'), 'E170未越界创建E171');
