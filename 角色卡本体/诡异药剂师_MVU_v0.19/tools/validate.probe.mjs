...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.12.json', '产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.12', '显示名正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === EVENT_IDS.length, '二百一十八事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E218', '事件锚点范围为E01-E218');
...
ok(contract.required.terminal_hook_event === 'E218' || contract.required.terminal_hook_event?.id === 'E218', 'E218为本版开放终点');
...
ok(conceptRouterContent.includes('E218'), '路由事件序列含E218');
...
for (const [index, eventId] of EVENT_IDS.entries()) {
  ...
  if (eventId === 'E218') ok(!content.includes('## 下一事件引入'), 'E218冻结终点不设下一事件引入');
...
ok(stage7ConceptEntries.length === 71, `v0.11概念UID${STAGE7_CONCEPT_UID_START}-${STAGE7_CONCEPT_UID_END}共71条（实际${stage7ConceptEntries.length}）`);
ok(stage7ConceptEntries.every((entry, index) => entry.id === 2000 + index
  && entry.extensions?.tavernweave?.logical_id === `C${691 + index}`), 'v0.11概念逻辑ID C691-C761与UID2000-2070逐项对应');
...
const allConceptEntries = [...stage1ConceptEntries, ...stage3ConceptEntries, ...stage4ConceptEntries, ...stage6ConceptEntries, ...stage7ConceptEntries];
for (const conceptEntry of allConceptEntries) {
  ...
  if (conceptTitle === '术语速查（E36-E64）') {
    ...
    ok(Array.isArray(eventIds) && eventIds.length > 0 && eventIds.every(id => EVENT_IDS.includes(id)), `${conceptEntry.comment}事件关联非空且只使用E01-E218`);
...
for (const character of CHARACTERS) {
  ...
  ok(eventIds.every(eventId => EVENT_IDS.includes(eventId)), `${character}事件元数据只使用E01-E218`);
...
ok(initial.元数据.卡名 === DISPLAY_NAME && initial.元数据.版本 === VERSION, '初始变量元数据为v0.11');
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.11')), '酒馆助手脚本命名含v0.11');
...
// 状态栏桥对（取消态不自动推进、E218无推进按钮、E64→E65前件挂E63）
...
ok(!statusUiText.includes("from: 'E218'"), 'E218无推进按钮');
...
ok(bridgePairs.length === 217, '全卡桥共217对');
ok(statusUiText.includes("from: 'E217', to: 'E218'"), '状态栏桥对覆盖E217→E218');
const statusBridgeCount = [...statusUiText.matchAll(/from: '(E\d{2,3})', to: '(E\d{2,3})'/g)].length;
ok(statusBridgeCount === 217, `状态栏桥对共217对（实际${statusBridgeCount}）`);
...
// ===== 开放终态收束 =====
ok(EVENT_IDS.includes('E218'), '开放终态事件E218已纳入事件序列');
const e218Content = await readText('src/events/E218_地狱来客自白与七使徒围攻.md');
ok(
  e218Content.includes('七使徒已经完成包围')
    && e218Content.includes('六名使徒发动进攻')
    && e218Content.includes('玩家明确选择后开启咒瞳')
    && e218Content.includes('任何攻击结果、伤亡、突围与胜负均尚未发生'),
  'E218严格停在七使徒包围、六人进攻、咒瞳玩家选择且战果未知',
);
ok(!e218Content.includes('E219') && !e218Content.includes('下一事件引入'), 'E218未越界创建E219或后继引入');
