...
ok(Object.keys(contract.required.optional_characters).length === 23, '应登记23名可选NPC');
...
for (let current = 318; current <= 540; current++) {
  const calls = [];
...
for (const entry of book.entries.filter(entry => entry.extensions?.tavernweave?.route_kind === 'optional_npc' || entry.id === 306)) {
  ...
  for (const state of ['未触发', '预兆', '活跃', '完成', '变形', '取消']) ok(typeof ejs.render(content, localsFor(540, state)) === 'string', `${entry.comment}状态渲染失败`);
}
