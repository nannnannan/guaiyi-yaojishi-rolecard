...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.19.json', '候选产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.19', '显示名正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 540, '五百四十事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E540', '事件锚点范围为E01-E540');
...
ok(contract.required.terminal_hook_event === 'E540' || contract.required.terminal_hook_event?.id === 'E540', 'E540为本版开放终点');
...
ok(conceptRouterContent.includes('E540'), '路由事件序列含E540');
...
for (const [index, eventId] of EVENT_IDS.entries()) {
  ...
  if (eventId === 'E540') ok(!content.includes('## 下一事件引入'), 'E540冻结终点不设下一事件引入');
...
for (const [name, npc] of optionalCharacters) {
  ...
  const compact = ['智脑', '视界之主', '老人', '夏娃', '康斯坦丁', '第一使徒', '孽主', '弑利亚', '大黑彘', '血娃娃与母亲', '未来画家', '艾泽法拉', '雷蒙·冬语', '以太', '光宴', '卡伦', '圣安娜', '星轨', '神王', '灭绝者'].includes(name);
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.19')), '酒馆助手脚本命名含v0.19');
...
ok(!statusUiText.includes("from: 'E540'"), 'E540无推进按钮');
...
ok(bridgePairs.length === 539, '全卡桥共539对');
ok(statusUiText.includes("from: 'E539', to: 'E540'"), '状态栏桥对覆盖E539→E540');
...
ok(statusBridgeCount === 539, `状态栏桥对共539对（实际${statusBridgeCount}）`);
...
// ===== 开放终态收束 =====
ok(EVENT_IDS.includes('E540'), '开放终态事件E540已纳入事件序列');
