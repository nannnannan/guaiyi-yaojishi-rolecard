...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.13.json', '产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.13', '显示名正确');
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 317, '三百一十七事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E317', '事件锚点范围为E01-E317');
...
ok(contract.required.terminal_hook_event === 'E317' || contract.required.terminal_hook_event?.id === 'E317', 'E317为本版开放终点');
...
ok(conceptRouterContent.includes('E317'), '路由事件序列含E317');
...
for (const [index, eventId] of EVENT_IDS.entries()) {
  ...
  if (eventId === 'E317') ok(!content.includes('## 下一事件引入'), 'E317冻结终点不设下一事件引入');
  ...
  // v0.13 阶段九事件默认走向按 300-1500（大事件至 2000+）标准写作，放宽上限至 3000
...
for (const conceptEntry of allConceptEntries) {
  ...
  if (conceptTitle === '术语速查（E36-E64）') {
    ...
    ok(Array.isArray(eventIds) && eventIds.length > 0 && eventIds.every(id => EVENT_IDS.includes(id)), `${conceptEntry.comment}事件关联非空且只使用E01-E317`);
...
for (const character of CHARACTERS) {
  ...
  ok(eventIds.every(eventId => EVENT_IDS.includes(eventId)), `${character}事件元数据只使用E01-E317`);
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.13')), '酒馆助手脚本命名含v0.13');
...
// 状态栏桥对（取消态不自动推进、E317无推进按钮、E64→E65前件挂E63）
...
ok(!statusUiText.includes("from: 'E317'"), 'E317无推进按钮');
...
ok(bridgePairs.length === 316, '全卡桥共316对');
ok(statusUiText.includes("from: 'E316', to: 'E317'"), '状态栏桥对覆盖E316→E317');
const statusBridgeCount = [...statusUiText.matchAll(/from: '(E\d{2,3})', to: '(E\d{2,3})'/g)].length;
ok(statusBridgeCount === 316, `状态栏桥对共316对（实际${statusBridgeCount}）`);
...
// ===== 开放终态收束 =====
ok(EVENT_IDS.includes('E317'), '开放终态事件E317已纳入事件序列');
const e317Content = await readText('src/events/E317_小树洞阶段九收束.md');
ok(
  e317Content.includes('小树洞')
    && e317Content.includes('母树')
    && e317Content.includes('终局封口')
    && e317Content.includes('严禁引出后续事件'),
  'E317严格停在玩家面对小树洞、尚未踏入且未越入母树契约执行',
);
ok(!e317Content.includes('## E318') && !e317Content.includes('E318·') && !e317Content.includes('下一事件引入'), 'E317未越界创建E318或后继引入');
