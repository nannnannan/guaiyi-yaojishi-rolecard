...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.15.json', '候选产物文件名正确');
ok(DISPLAY_NAME === '《诡异药剂师》v0.15', '显示名正确');
...
for (const [name, npc] of optionalCharacters) {
  ...
  const compact = ['智脑', '视界之主', '老人'].includes(name);
...
ok(initial.元数据.卡名 === DISPLAY_NAME && initial.元数据.版本 === VERSION, '初始变量元数据为v0.15');
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.15')), '酒馆助手脚本命名含v0.15');
