...
// 世界书元数据从 manifest 派生，消灭源码残留的旧版名称/版本
worldbook.name = cardSource.name;
worldbook.extensions = {
  ...worldbook.extensions,
  tavernweave: {
    ...worldbook.extensions?.tavernweave,
    id: manifest.worldbook.stable_id,
    version: manifest.worldbook.version,
  },
};

const scheduleEntry = worldbook.entries.find(entry => entry.comment === '[核心]自主世界事件调度');
if (scheduleEntry) {
  const detailBlocks = [];
  const materialStart = contract.required?.event_context_windows?.material_entry_start ?? 700;
  for (let index = 0; index < contract.required.event_ids.length; index += 1) {
    const eventEntry = worldbook.entries.find(entry => entry.id === materialStart + index);
    if (!eventEntry) continue;
    let detail = eventEntry.content;
    const bridgeAt = detail.indexOf('## 下一事件引入');
    if (bridgeAt !== -1) {
      detail = `${detail.slice(0, bridgeAt)}\n<%_ } _%>`;
    }
    const stageThreeHookAt = detail.indexOf('## 阶段待续钩子');
    if (stageThreeHookAt !== -1) {
      detail = `${detail.slice(0, stageThreeHookAt)}\n<%_ } _%>`;
    }
    const eventId = `E${String(index + 1).padStart(2, '0')}`;
    detailBlocks.push(`<%_ if (detailWindow.indexOf('${eventId}') !== -1) { _%>\n${detail}\n<%_ } _%>`);
  }
  scheduleEntry.content = `${scheduleEntry.content}\n\n## 事件详情窗口（按状态展开）\n\n${detailBlocks.join('\n\n')}\n`;
}
...
const [
  ...
  ...(manifest.card.alternate_greetings ?? []).map(file => readText(file)),
]);
...
const data = {
  ...
  description,
  personality: '',
  scenario: '觉醒日午后，血锯药剂店：血娃娃的诊疗刚刚结束，右眼抵押了诊金，寻母任务完成度2%；E01已经完成，E02处于收尾活跃态，E03尚未触发；柜台上的盘子里放着从她胃中取出的器官。',
