...
const BIG_ANCHORS = new Set(['E47', 'E60', 'E61', 'E63', 'E77', 'E80', 'E94', 'E224', 'E226', 'E228', 'E236', 'E239', 'E242', 'E245', 'E253', 'E254', 'E257', 'E260', 'E261', 'E265']);
...
// ===== 契约关键结构 =====
ok(EVENT_IDS.length === 265, '二百六十五事件锚点');
ok(EVENT_IDS[0] === 'E01' && EVENT_IDS[EVENT_IDS.length - 1] === 'E265', '事件锚点范围为E01-E265');
...
ok(contract.required.terminal_hook_event === 'E265' || contract.required.terminal_hook_event?.id === 'E265', 'E265为本版开放终点');
...
ok(conceptRouterContent.includes('E265'), '路由事件序列含E265');
...
for (const [index, eventId] of EVENT_IDS.entries()) {
  ...
  if (eventId === 'E265') ok(!content.includes('## 下一事件引入'), 'E265冻结终点不设下一事件引入');
  ...
  const upper = BIG_ANCHORS.has(eventId) ? 2000 : 1200;
...
ok(helperSource.every(script => String(script.name ?? '').includes('v0.12')), '酒馆助手脚本命名含v0.12');
...
ok(!statusUiText.includes("from: 'E265'"), 'E265无推进按钮');
...
ok(bridgePairs.length === 264, '全卡桥共264对');
ok(statusUiText.includes("from: 'E264', to: 'E265'"), '状态栏桥对覆盖E264→E265');
const statusBridgeCount = [...statusUiText.matchAll(/from: '(E\d{2,3})', to: '(E\d{2,3})'/g)].length;
ok(statusBridgeCount === 264, `状态栏桥对共264对（实际${statusBridgeCount}）`);
...
// ===== 开放终态收束 =====
ok(EVENT_IDS.includes('E265'), '开放终态事件E265已纳入事件序列');
const e265Content = await readText('src/events/E265_公海前奏克苏鲁注视.md');
ok(
  e265Content.includes('炽天使的魂灯')
    && e265Content.includes('克苏鲁的注视')
    && e265Content.includes('终局封口')
    && e265Content.includes('严禁引出后续事件'),
  'E265严格停在梵蒂冈深层探索前、魂灯点亮且未越入旧日核心',
);
ok(!e265Content.includes('E266') && !e265Content.includes('下一事件引入'), 'E265未越界创建E266或后继引入');
