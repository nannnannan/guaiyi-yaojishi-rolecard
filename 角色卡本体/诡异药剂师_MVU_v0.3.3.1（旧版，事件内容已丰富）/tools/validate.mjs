...
ok(manifest.version === '0.3.3.1', 'manifest版本为0.3.3.1');
...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.3.3.1.json', '产物文件名正确');
ok(cardSource.name === '《诡异药剂师》v0.3.3.1', '显示名正确');
...
ok(packed.data?.character_version === '0.3.3.1', 'data版本正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.id === manifest.worldbook.stable_id, '打包世界书稳定ID正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.version === '0.3.3.1', '打包世界书版本正确');
...
ok(initial.事件.锚点状态.E01.状态 === '活跃', 'E01初始活跃收尾');
ok(initial.事件.锚点状态.E02.状态 === '未触发', 'E02初始未触发（纯事件驱动）');
ok(initial.事件.锚点状态.E01.收尾 === true, 'E01初始收尾（诊疗已完成）');
ok(initial.事件.锚点状态.E02.收尾 === false, 'E02初始未收尾');
...
for (const token of ['《诡异药剂师》v0.3.3.1', '0.3.3.1', '收尾', '恶堕', '左左', '血锯', '血衣女士', '小小', '人偶夫人', '泰坦头颅', '巫神头颅', '小宝贝']) {
...
ok(helperNames.includes('v0.3.3.1'), '酒馆助手脚本命名含v0.3.3.1');
...
ok(openingInitVar.事件.锚点状态.E01.状态 === '活跃', '开场白initvar E01活跃收尾');
ok(openingInitVar.事件.锚点状态.E02.状态 === '未触发', '开场白initvar E02未触发');
...
ok(mainlineText.includes('E12收束后不设计续接'), '上下文窗口明确E12收束边界');
...
const bridgePairs = [['E01','E02'],['E02','E03'],['E03','E04'],['E04','E05'],['E05','E06'],['E06','E07'],['E07','E08'],['E08','E09'],['E09','E10'],['E10','E11'],['E11','E12']];
for (const [from, to] of bridgePairs) {
  ok(mainlineText.includes(`### ${from}→${to}`), `即时衔接段包含${from}→${to}`);
}
ok(!mainlineText.includes('### E12→'), '衔接段不包含E12后续');
ok(mainlineText.includes('猪类') && mainlineText.includes('拍门'), 'E01→E02引子token齐全');
ok(mainlineText.includes('超出门框'), 'E02→E03引子token齐全');
ok(mainlineText.includes('求援信'), 'E08→E09引子token齐全');
...
ok(statusUiText.includes('"E01":') && statusUiText.includes('"状态":  "活跃"'), '状态栏回退状态E01活跃');
ok(statusUiText.includes('"E02":') && statusUiText.includes('"状态":  "未触发"'), '状态栏回退状态E02未触发');
ok(statusUiText.includes('"事件ID":  "E01"'), '状态栏回退有活跃事件');
