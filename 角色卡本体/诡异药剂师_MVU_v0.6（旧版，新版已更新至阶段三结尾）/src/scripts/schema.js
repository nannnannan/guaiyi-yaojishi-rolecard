...
async function loadSchemaBridge() {
  ...
  for (const url of BRIDGE_URLS) {
    try {
      ...
      lastError = error;
      console.warn(`[诡异药剂师v0.6] Zod 桥接加载失败，尝试备用源：${url}`, error);
    }
  }
...
const Schema = z.object({
  元数据: z.object({
    卡名: z.literal('《诡异药剂师》v0.6'),
    版本: z.literal('0.6.0'),
  }),
  世界: z.object({
    阶段编号: z.enum(['S0', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7']),
    阶段名称: z.enum(['药店接管期', '怪物客户建立期', '血衣与药剂风波期', '魔人协会冲突期', '猎头者家庭与师徒归来期', '幸福之家与独立行医期', '夜医启程与寂静镇灾变期', '黑夜城授职与跨界伏笔期']),
  ...
  关系: z.object({
    ...
    人偶夫人: relationship(),
    爱丽丝: relationship({ evilLocked: true }),
  ...
  角色关系: z.array(relationEdge).max(48),
  系统: z.object({
    当前任务: shortText,
    任务阶段: shortText,
  ...
  if (data.关系.爱丽丝.恶堕 !== 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['关系', '爱丽丝', '恶堕'],
      message: '爱丽丝当前幼态外观下恶堕恒为0；非性恋爱与吸引另行记录',
    });
  }
});
