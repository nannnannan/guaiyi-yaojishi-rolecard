...
ok(manifest.version === '0.31.0', 'manifest版本为0.31.0');
...
ok(manifest.packed_json === 'dist/诡异药剂师_v0.31.json', '产物文件名正确');
ok(cardSource.name === '《诡异药剂师》v0.31', '显示名正确');
...
ok(packed.data?.character_version === '0.31.0', 'data版本正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.id === manifest.worldbook.stable_id, '打包世界书稳定ID正确');
ok(packed.data?.character_book?.extensions?.tavernweave?.version === '0.31.0', '打包世界书版本正确');
...
ok(!packedText.includes('"character_version": "0.3.0"'), '产物不残留旧角色卡版本面');
...
for (const token of ['《诡异药剂师》v0.31', '0.31.0', '恶堕', '左左', '血锯', '血衣女士', '小小', '人偶夫人', '泰坦头颅', '巫神头颅', '小宝贝']) {
...
ok(helperNames.includes('v0.31'), '酒馆助手脚本命名含v0.31');
...
for (const token of ['血娃娃', '<StatusPlaceHolderImpl/>', '门栓没有自行打开', '系统已完成最低限度启动']) {
  ok(openingText.includes(token), `开场白包含：${token}`);
}
ok(!openingText.includes('【可选方向】') && !openingText.includes('可选方向'), '开场白不含可选方向列表');
ok(!openingText.includes('yyds') && !openingText.includes('绝绝子'), '开场白不含禁用网络梗');
