...
**仅使用 MVU 时需要**。最简单的方式：直接在 `正则/状态栏界面.html` 中编写 HTML 与宏，无需独立前端项目、无需构建工具。渲染时 `状态栏界面` 正则脚本（已通过 MVU patch 注册）将消息中的 `<StatusPlaceHolderImpl/>` 占位符替换为 `正则/状态栏界面.html` 的内容，宏由 SillyTavern 替换为变量值。占位符由打包工具在打包时自动追加到 MVU 项目的 `.txt` 开场白末尾（`.md` 开场白不追加）。
