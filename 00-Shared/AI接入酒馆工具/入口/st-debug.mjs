...
//
// 监听参考：写卡知识库 06_事件监听 / 01_消息楼层操作，事件名以本机
// ST 1.17.0 运行时探测为准（见 手册/酒馆运行时速查.md）。
//
// 环境变量：
...
选项:
  ...
  --capture                   捕获本轮完整请求负载（系统提示/预设/世界书/历史/输入）
                              与模型回复，写入 提示词监听/captures/（含 viewer 兼容导出）
...
async function finishCapture(page, meta, flags, waitMs = 6000) {
  ...
  const paths = promptCapture.writeCaptureFiles(turn, flags.captureDir || join(__dirname, '../提示词监听/captures'));
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case 'log': {
        const text = tailLog(Number(args[0]) || 50);
        if (flags.json) console.log(JSON.stringify({ lines: Number(args[0]) || 50, content: text }));
