...
export async function getGenerationState(page) {
  return page.evaluate(() => {
    ...
    const stopVisible = Boolean(stopButton && window.getComputedStyle(stopButton).display !== 'none');
    const streaming = Boolean(ctx.streamingProcessor && ctx.streamingProcessor.isFinished === false);
    return { active: stopVisible || streaming, stopVisible, streaming };
  });
}
...
// Read stored configuration objects directly from the connected page context;
...
export async function stopGeneration(page) {
  return page.evaluate(async () => {
    ...
    const stopVisible = Boolean(stopButton && window.getComputedStyle(stopButton).display !== 'none');
    const streaming = Boolean(ctx.streamingProcessor && ctx.streamingProcessor.isFinished === false);
    if (!stopVisible && !streaming) {
      return { stopped: false, reason: '当前页面没有进行中的生成' };
    }
...
// Use the host deletion lifecycle rather than mutating ctx.chat directly:
