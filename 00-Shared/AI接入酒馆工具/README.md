...
3. **页面保存剥掉世界书/提示字段**：ST 保存浅层角色会把 `data.character_book` 写没，还可能在导入器收尾重建之后异步把 `system_prompt` / `post_history_instructions` 存成空串（v0.5、v0.6 两次真机踩坑）；
   改卡后用导入器收尾重建即可。导入器现已内置收尾读回校验（延时读回、被剥空自动以源卡重建、最多 3 次，失败在报告给警告），导入后请确认输出中「提示字段校验」为通过。
