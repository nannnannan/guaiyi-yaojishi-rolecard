...
- **已分卷 / 已分文件**：按原有卷/文件分别处理，**不合并**。每个卷或文件单独作为处理单元，保留原有边界。
- **未分卷但属于超长小说**：如果章节序号有卷级重置，或材料规模明显超出单次上下文容量，用 bash 按卷分割后再处理。示例：

```bash
# 按卷标题边界分割（根据实际卷名调整）
awk '/^第一卷/{flag=1} /^第二卷/{flag=0} flag' 源材料.txt > 第一卷.txt
```

分割后按卷分别构建大纲，最后在合并整理阶段按卷顺序汇总到同一份 `故事大纲.yaml`。
...
- **分卷优先**：尽量按卷分配，保持卷内完整性
- **上下文容量检查**：逐卷检查子代理上下文容量，确认当前卷能否在一次任务中完整处理；如果超限，在卷内按完整章节边界再分批
...
# 关键信息确认结果（整理深化后、完成 design-spec.md 前，由关键信息确认阶段补充）
confirmed_info:
  地名类:
    - 问题: "..."
      答案: "..."
