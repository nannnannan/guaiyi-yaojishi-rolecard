...
// 依赖：
//   - Node.js 18+
//   - Playwright（优先项目内安装；本机回退到 Bundled Runtime 自带包）
...
//   node 入口/st-bridge.mjs worldbook 《诡异药剂师》v0.4 --full
//   node 入口/st-bridge.mjs switch 《诡异药剂师》v0.4
...
命令:
  ...
  screenshot [路径]          保存页面截图（默认 AI接入酒馆工具/screenshots/）
  help                       显示本帮助
    ...
    switch (command) {
      ...
      case 'send': {
        ...
        out(d, flags, (x) => {
          ...
          else if (x.generation?.queued) lines.push('已触发生成（未等待）。');
      ...
      case 'wait': {
        const d = await session.waitForGeneration(page, flags.timeout || 180000);
        out(d, flags, (x) => x.ended ? '生成已结束。' : '等待超时（可能没有进行中的生成）。');
        break;
      }
      case 'stop': {
      ...
      case 'screenshot': {
        const target = args[0] || join(__dirname, '..', 'screenshots', `st-${Date.now()}.png`);
