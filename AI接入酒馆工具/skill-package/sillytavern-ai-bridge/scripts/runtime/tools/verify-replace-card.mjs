...
const replacementBackupRoot = join(outputRoot, 'backups', 'card-replace');
const oldName = `Bridge临时替换旧卡-${token}`;
const newName = `Bridge临时替换新卡-${token}`;
const newerName = `Bridge临时替换再升级卡-${token}`;
const guardName = `Bridge临时替换共享引用卡-${token}`;
const oldBook = `Bridge临时替换旧世界书-${token}`;
const newBook = `Bridge临时替换新世界书-${token}`;
const newerBook = `Bridge临时替换再升级世界书-${token}`;
const controlBook = `Bridge临时替换无关世界书-${token}`;
...
async function main() {
  ...
  try {
    ...
    for (const backupDir of backupDirs) {
      const resolvedBackupDir = backupDir ? resolve(backupDir) : '';
      if (resolvedBackupDir.startsWith(`${replacementBackupRoot}${sep}`) && existsSync(resolvedBackupDir)) {
        rmSync(resolvedBackupDir, { recursive: true, force: true });
      }
    }
