...
const require=createRequire(path.join(workspace,'角色卡本体/诡异药剂师_MVU_v0.18/package.json'));
...
ok(JSON.stringify(ledger.chapter_range)==='[1927,2086]'&&JSON.stringify(ledger.line_range)==='[165861,179813]','原文范围冻结');
let nextLine=165861,nextChapter=1927;
...
ok(nextLine===179814&&nextChapter===2087,'33包无空洞覆盖完整');
...
ok(ledger.events.length===48&&ledger.events.every((x,i)=>x.id===`E${493+i}`),'事件编号连续');
...
const metadataBan=/第\s*[\d一二三四五六七八九十百千]+\s*章|小总结|大总结|阶段[一二三四五六七八九十]|v0\.1[6789]|编制说明|来源台账|核对记录|原作/u;
...
for(const event of ledger.events){
 ...
 if(event.id==='E540')ok(!body.includes('下一事件引入')&&!body.includes('E541'),'E540开放终点');
 ...
 if(strict){ok(Array.isArray(event.source_lines)&&event.source_lines[0]>=165861&&event.source_lines[1]<=179813,`${event.id}原文行证据`);ok(event.source_chapters?.[0]>=1927&&event.source_chapters?.[1]<=2086,`${event.id}章节证据`);}
}
if(strict){let line=165861;for(const event of ledger.events){ok(event.source_lines?.[0]===line,`${event.id}事件原文覆盖连续`);line=event.source_lines?.[1]+1;}ok(line===179814,'全部事件源行覆盖终点');}
...
if(manifest){
 ...
 for(const item of registered){
  ...
  ok(item.source_lines.length>0&&item.source_lines.every(s=>s[0]>=165861&&s[1]<=179813&&s[1]>=s[0]),`${item.output}原文行范围`);
  if(item.id){
   ...
   ok(item.kind==='update'||Number(item.id.slice(1))>=1389,`${item.id}新增编号不沿用旧切片`);
  }
 ...
 if(strict){
  ...
  ok(manifest.character_coverage?.length===46&&manifest.character_coverage.every(x=>x.status!=='pending'),'46名既有人物已筛查');
 }
}
if(exists('摘要/事件摘要.json')){
 ...
 ok(summaries.length===48&&summaries.every((s,i)=>s.id===ledger.events[i].id&&(s.blank?s.text==='':Boolean(s.text))),'48条摘要与留白一致');
 ...
 ok(bridges.terminal==='E540'&&!JSON.stringify(bridges).includes('E541'),'衔接终点封口');
