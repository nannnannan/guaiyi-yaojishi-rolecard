...
// Scan 1927–2087 so chapter 2086 has a known end line (start of 2087 − 1).
lines.forEach((line,i)=>{const m=line.match(/^第(\d+)章/);if(m&&+m[1]>=1927&&+m[1]<=2087)heads.push({chapter:+m[1],line:i+1,title:line});});
for(let n=1927;n<=2087;n++)if(heads.filter(x=>x.chapter===n).length!==1)throw new Error('章节异常 '+n);
const lineOf=n=>heads.find(x=>x.chapter===n).line;
const groups=[{id:'A',chapters:[1927,1980],events:[493,508]},{id:'B',chapters:[1981,2033],events:[509,524]},{id:'C',chapters:[2034,2086],events:[525,540]}];
const packets=[];
...
const baselines=['角色卡本体/诡异药剂师_MVU_v0.18','角色卡设定/v0.18工作区','角色卡设定/阶段十至阶段十八_概念事件切片/阶段14_第1927-2086章'].map(rel=>{
...
const candidate='角色卡设定/阶段十至阶段十八_概念事件切片/阶段14_第1927-2086章/事件';
...
const ledger={source:'角色卡设定/原文.txt',normalization:'UTF-8 remove BOM; CRLF to LF; packet ends in LF',normalized_sha256:sha(source),chapter_range:[1927,2086],line_range:[lineOf(1927),lineOf(2087)-1],groups,packets,baselines,events};
fs.writeFileSync(path.join(root,'来源台账.json'),JSON.stringify(ledger,null,2)+'\n');
fs.writeFileSync(path.join(root,'事件蓝图.json'),JSON.stringify({status:'candidate_boundaries_to_verify',scope:[493,540],groups,events:events.map(({id,title,stage,group,candidate,output})=>({id,title,stage,group,candidate,output,previous:+id.slice(1)===493?'E492':`E${+id.slice(1)-1}`,next:+id.slice(1)===540?null:`E${+id.slice(1)+1}`,boundary_rule:'本组逐包核实后记录精确原文边界，不把后件揭示倒灌前件'}))},null,2)+'\n');
