...
// Scan 1768–1927 so chapter 1926 has a known end line (start of 1927 − 1).
lines.forEach((line,i)=>{const m=line.match(/^第(\d+)章/);if(m&&+m[1]>=1768&&+m[1]<=1927)heads.push({chapter:+m[1],line:i+1,title:line});});
for(let n=1768;n<=1927;n++)if(heads.filter(x=>x.chapter===n).length!==1)throw new Error('章节异常 '+n);
const lineOf=n=>heads.find(x=>x.chapter===n).line;
const groups=[{id:'A',chapters:[1768,1820],events:[435,450]},{id:'B',chapters:[1821,1873],events:[451,466]},{id:'C',chapters:[1874,1926],events:[467,492]}];
const packets=[];
...
const baselines=['角色卡本体/诡异药剂师_MVU_v0.17','角色卡设定/v0.17工作区','角色卡设定/阶段十至阶段十八_概念事件切片/阶段13_第1768-1926章'].map(rel=>{
...
const candidate='角色卡设定/阶段十至阶段十八_概念事件切片/阶段13_第1768-1926章/事件';
...
const ledger={source:'角色卡设定/原文.txt',normalization:'UTF-8 remove BOM; CRLF to LF; packet ends in LF',normalized_sha256:sha(source),chapter_range:[1768,1926],line_range:[lineOf(1768),lineOf(1927)-1],groups,packets,baselines,events};
fs.writeFileSync(path.join(root,'来源台账.json'),JSON.stringify(ledger,null,2)+'\n');
fs.writeFileSync(path.join(root,'事件蓝图.json'),JSON.stringify({status:'candidate_boundaries_to_verify',scope:[435,492],groups,events:events.map(({id,title,stage,group,candidate,output})=>({id,title,stage,group,candidate,output,previous:+id.slice(1)===435?'E434':`E${+id.slice(1)-1}`,next:+id.slice(1)===492?null:`E${+id.slice(1)+1}`,boundary_rule:'本组逐包核实后记录精确原文边界，不把后件揭示倒灌前件'}))},null,2)+'\n');
