...
const require=createRequire(path.join(workspace,'角色卡本体/诡异药剂师_MVU_v0.16/package.json'));
...
ok(JSON.stringify(ledger.chapter_range)==='[1611,1767]'&&JSON.stringify(ledger.line_range)==='[141073,153711]','原文范围冻结');
let nextLine=141073,nextChapter=1611;
...
ok(nextLine===153712&&nextChapter===1768,'33包无空洞覆盖完整');
...
ok(ledger.events.length===38&&ledger.events.every((x,i)=>x.id===`E${397+i}`),'事件编号连续');
...
const metadataBan=/第\s*[\d一二三四五六七八九十百千]+\s*章|小总结|大总结|阶段[一二三四五六七八九十]|v0\.1[67]|编制说明|来源台账|核对记录|原作/u;
const contentFiles=['事件','概念','人物增量','NPC'].flatMap(d=>walk(path.join(root,d)).filter(f=>f.endsWith('.md')));
...
for(const event of ledger.events){
 ...
 if(event.id==='E434')ok(!body.includes('下一事件引入')&&!body.includes('E435'),'E434开放终点');
 ...
 if(strict){ok(Array.isArray(event.source_lines)&&event.source_lines[0]>=141073&&event.source_lines[1]<=153711,`${event.id}原文行证据`);ok(event.source_chapters?.[0]>=1611&&event.source_chapters?.[1]<=1767,`${event.id}章节证据`);}
}
if(strict){let line=141073;for(const event of ledger.events){ok(event.source_lines?.[0]===line,`${event.id}事件原文覆盖连续`);line=event.source_lines?.[1]+1;}ok(line===153712,'全部事件源行覆盖终点');}
...
if(manifest){
 ...
 for(const item of registered){
  ...
  ok(item.source_lines.length>0&&item.source_lines.every(s=>s[0]>=141073&&s[1]<=153711&&s[1]>=s[0]),`${item.output}原文行范围`);
  if(item.id){
   ...
   ok(item.kind==='update'||Number(item.id.slice(1))>=1284,`${item.id}新增编号不沿用旧切片`);
  }
 ...
 if(strict){
  ...
  ok(manifest.character_coverage?.length===34&&manifest.character_coverage.every(x=>x.status!=='pending'),'34名既有人物已筛查');
 }
}
if(exists('摘要/事件摘要.json')){
 ...
 ok(summaries.length===38&&summaries.every((s,i)=>s.id===ledger.events[i].id&&(s.blank?s.text==='':Boolean(s.text))),'38条摘要与留白一致');
 ...
 ok(bridges.terminal==='E434'&&!JSON.stringify(bridges).includes('E435'),'衔接终点封口');
